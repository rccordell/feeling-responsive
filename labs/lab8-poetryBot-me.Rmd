...
```{r}
moby <- paste(scan(file = "mobydick.txt", what = "char")) %>%
  gutenberg_strip() %>%
  as_data_frame() %>%
  rename(word = value)
moby$word <- gsub("moby-\\d\\d\\d.txt", "", moby$word, perl=TRUE)
  
#  gutenberg_works(title == "Moby Dick") %>%
#  gutenberg_download(gutenberg_id = "title", mirror = "http://gutenberg.readingroo.ms/") %>%
#  gutenberg_strip() %>%
#  filter(text != "")
```
...
We could use these random walks to create a twitter bot, following the directions I will outline below. You'd just need to write a modification limiting the walk to fewer than 140 characters.
...
Finally, you can use the generator you just created to make a chain, switching the starting word as you please:

```{r}
set.seed(33)
generator(c(sample(mobywords$word, 1)),24)
```

Or you could sample a random starting word:
```{r}
set.seed(33)
generator(c(sample(mobywords$word, 1)),24)
```
...
```{r}
setup_twitter_oauth("aSF8QehYl8xXeE9JOuEaaUuXk", 
                    "Pf0JJ1na069RNuWXUtJ0AykQBg7qSFlNkU5idiPjb5BBVBPfF1", 
                    '2871785039-DM9YqDX1pFNrrE7sgyFvVZxa0qmTxsUamAJc6Lv', 
                    'TmlXr7aVqqTNTRyVmTAjCwLiwnXke7rYLv5cH5PbKj82O')
...
poem <- paste(c("Call me ", poem_word("proper-noun"), ". Some years ago, never mind how ", poem_word("preposition")), collapse = "")
cat(poem)
